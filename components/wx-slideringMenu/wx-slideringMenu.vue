				...
				<view class="_tab-item-box" :class="[defaultConfig.itemWidth ? '_clamp' : '_flex']">
					<block v-for="(item, index) in tabList" :key="index" >
						<view 
							...
							{{ item.name }}
							<view
								v-if="isNumber(item.badge) && parseInt(item.badge) !== 0"
								class="_badge">
								{{parseInt(item.badge) >= 1000 ? '...' : parseInt(item.badge)}}
							</view>
						</view>
					</block>
				</view>
				<!-- <image class="_underline" src="../static/imgs/biaoti_sel@2x.png" :style="{transform: 'translateX(' + slider.left + 'px)'}"></image> -->
			</view>
		</scroll-view>
	</view>
</template>
